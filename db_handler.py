...
def write_to_database(info, escrow_wallet):
    ...
    sqlite_insert_with_param = """INSERT INTO loans
                      (name, email, story, location, wallet, escrow_wallet, max_fees, sum, photo, risk, loan_duration, status) 
                      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?);"""

    data_tuple = (info['inputName'], info['inputEmail'], info['inputStory'], info['inputLocation'], info['inputWallet'], escrow_wallet,
                  info['inputMaxFees'], info['inputSum'], "", info['inputRisk'], info['inputDuration'], "OPEN")
